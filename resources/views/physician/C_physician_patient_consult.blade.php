			                ...
			                <div id="step-3" class="">
			                    <div id="physical-examination" style="float:left; margin-top: 10px; width: 100%;">
									...
									<div style="float:left; font-size:15px; width:300px;">
										...
										<input type="number" name="eyesWithGlassesTextArea" class="" style="width:350px; border-radius:8px;" disabled value="@if($medicalHistory){{ $medicalHistory->eyeGlassesGrade }}@endif" data-parsley-min="0">
									</div>
        ...
        var btnFinish = $('<button></button>').text('Finish')
                            ...
                            .on('click', function(e){ 
								...
								if ($('#saveForm').parsley().isValid() && $('#saveForm').data('rec') == 0){
									...
									$.ajax({
										url: '/physician/update/medical/history/' + $('#saveForm').data('id'),
										type: 'get',
										data: $('#saveForm').serialize(),
										success: function(output){
											swal({
